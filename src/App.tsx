...
import { MainMenu, HUD, UpgradeMenu, GameOver, StorySceneRenderer, TutorialOverlay, isTutorialComplete, SettingsMenu, AccountMenu } from '@/components'
import { AuthProvider } from '@/contexts/AuthContext'

export const App = () => {
  const [phase, setPhase] = useState<GamePhase>('menu')
  const [gameState, setGameState] = useState<GameState>({
    score: 0, health: 100, maxHealth: 100, wave: 1,
    gameOver: false, isPaused: false, isPlaying: false,
    ...
    upgradeData: {}, bugsKilled: 0, combo: 0, crystals: 0,
  })
  ...
  const handleStartGame = useCallback(() => {
    ...
    engine.onWaveComplete = () => { setPhase('upgrade') }
    engine.onGameOver = (score) => { setGameOverScore(score); setGameOverWave(engine.state.wave); setGameState(prev => ({ ...prev, score, gameOver: true, isPlaying: false })); setPhase('gameOver') }
    engine.onStoryScene = (scene) => { if (scene) { setCurrentScene(scene); setPhase('story'); engine.pause() } }
      ...
      {phase === 'paused' && (
        <div className="absolute inset-0 flex items-center justify-center z-20 bg-black/70 backdrop-blur-sm">
          <div className="flex flex-col items-center space-y-8 p-10">
            ...
            <button
              onClick={() => { if (engineRef.current) { engineRef.current.destroy(); engineRef.current = null } setPhase('menu') }}
              className="px-10 py-3 border border-white/10 hover:border-white/20 text-white/40 font-mono text-xs tracking-widest transition-all"
            >
              QUIT TO MENU
            </button>
