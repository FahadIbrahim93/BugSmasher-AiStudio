import { Skull, RotateCcw, Home, Trophy, Zap, Gift, Share2 } from 'lucide-react';
...
export function GameOver({ score, wave, onRetry, onMainMenu }: { score: number, wave: number, onRetry: () => void, onMainMenu: () => void }) {
  ...
  useEffect(() => {
    const trackHigh = async () => {
      ...
      // Track top 5 local scores
      HighScoreManager.submitScore(score, wave);

      // Submit to global cloud leaderboard if authenticated
      try {
  ...
  return (
    <div className="absolute inset-0 bg-black/90 backdrop-blur-3xl flex flex-col items-center justify-center z-50 p-4">
      ...
      <div className="max-w-md w-full text-center space-y-12">
        ...
        {/* Social Sharing Transmission Block */}
        <div className="bg-zinc-950/60 p-4 border border-white/5 rounded-2xl space-y-3">
          <p className="text-[8px] text-zinc-500 font-mono uppercase tracking-widest leading-none">
            TRANSMIT FIELD LOGS TO MAIN PUBLIC SENSORS
          </p>
          <div className="grid grid-cols-3 gap-2">
            <button
              onClick={() => {
                soundManager.uiClick();
                const shareText = `I reached Wave ${wave} with a high score of ${score.toLocaleString()} on BUGSMASHER! Defend the core, smash the swarm! 🚀🐛 #BUGSMASHER`;
                const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}`;
                window.open(url, '_blank');
              }}
              className="py-2 bg-black border border-white/10 hover:border-[#1da1f2]/30 rounded-xl flex items-center justify-center transition-all text-[11px] font-bold font-mono text-zinc-400 hover:text-cyan-400 hover:border-cyan-400"
            >
              X/TWITTER
            </button>
            <button
              onClick={() => {
                soundManager.uiClick();
                const title = `I reached Wave ${wave} with ${score.toLocaleString()} on BUGSMASHER!`;
                const text = `BUGSMASHER Field Report:\n- Clear Wave Segment: ${wave}\n- Secure High Score: ${score.toLocaleString()}\nJoin the Swarm Smashing Campaign!`;
                const url = `https://www.reddit.com/submit?title=${encodeURIComponent(title)}&text=${encodeURIComponent(text)}`;
                window.open(url, '_blank');
              }}
              className="py-2 bg-black border border-white/10 hover:border-[#ff4500]/30 rounded-xl flex items-center justify-center transition-all text-[11px] font-bold font-mono text-zinc-400 hover:text-[#ff4500] hover:border-[#ff4500]"
            >
              REDDIT
            </button>
            <button
              onClick={() => {
                soundManager.uiClick();
                const shareUrl = window.location.href || 'https://ai.studio/build';
                const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`;
                window.open(url, '_blank');
              }}
              className="py-2 bg-black border border-white/10 hover:border-[#1877f2]/30 rounded-xl flex items-center justify-center transition-all text-[11px] font-bold font-mono text-zinc-400 hover:text-[#1877f2] hover:border-[#1877f2]"
            >
              FACEBOOK
            </button>
          </div>
