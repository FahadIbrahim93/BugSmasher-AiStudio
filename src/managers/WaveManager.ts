...
export class WaveManager {
  ...
  startWave() {
    ...
    if (this.isBossWave) this.bugsToSpawn = 1 + Math.floor(this.engine.state.wave * 1.5)
      ...
      this.bugsToSpawn =
        GameConfig.waves.baseBugs + this.engine.state.wave * GameConfig.waves.bugsPerWave + Math.floor(this.engine.performanceFactor * 5)
  ...
  update(dt: number) {
    ...
    if (this.bossIntroActive) {
      ...
      if (this.bossIntroTimer < 2 && !this.bossWarningSounded) {
    ...
    if (this.bugsToSpawn > 0) {
      ...
      let rate = Math.max(
        ...
        GameConfig.waves.baseSpawnRate - this.engine.state.wave * GameConfig.waves.spawnRateReduction,
      )
      if (this.isBossWave) {
        rate = 1.7
        if (!this.bossSpawned) {
  ...
  private spawnBoss() {
    ...
    if (GameConfig.bugs.boss.variants) {
      ...
      const v = GameConfig.bugs.boss.variants[idx]
      bug.variantId = v.id
      bug.color = v.color
    }
  ...
  spawnSpecificMinion(x: number, y: number) {
    ...
    const bug = this.createBug(type, this.engine.state.wave)
    bug.x = x
    bug.y = y
    this.engine.bugs.push(bug)
  ...
  private createBug(typeName: string, wave: number): Bug {
    ...
    if (edge === 0) {
      ...
      x = this.engine.width + m
      y = Math.random() * this.engine.height
    } else if (edge === 2) {
      x = Math.random() * this.engine.width
      y = this.engine.height + m
    } else {
      x = -m
      y = Math.random() * this.engine.height
    }
