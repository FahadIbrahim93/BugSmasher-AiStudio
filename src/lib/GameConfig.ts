export interface BugStatConfig {
  color: string; baseSpeed: number; speedPerWave: number
  size: number; score: number; baseHp: number; hpPerWave: number
}

export interface BossConfig extends BugStatConfig {
  attackRate: number; minionSpawnCount: number; glitchChance: number
  barrageRate: number; barrageCount: number; barrageWarningTime: number
  barrageRadius: number; shieldDuration: number
...
export const GameConfig = {
  ...
  bugs: {
    ...
    boss: { 
      color: '#ff2255', baseSpeed: 20, speedPerWave: 2, size: 60, score: 5000, baseHp: 100, hpPerWave: 50,
      attackRate: 3.5, minionSpawnCount: 4, glitchChance: 0.15,
      barrageRate: 8.0, barrageCount: 5, barrageWarningTime: 1.5,
      barrageRadius: 80, shieldDuration: 6.0,
      variants: [
