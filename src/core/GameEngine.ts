...
import { ParticleSystem, Renderer, soundManager, GameConfig, saveManager, upgradeSystem, achievementSystem, assetManager } from '@/lib'
import type { UpgradeId } from '@/lib'
...
export class GameEngine {
  ...
  dashTargetX = 0
  dashTargetY = 0
  readonly dashDuration = 0.15
  readonly dashCooldown = 3
  readonly dashDistance = 180
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.state = {
      score: 0, health: GameConfig.player.maxHealth, maxHealth: GameConfig.player.maxHealth,
      wave: 1, gameOver: false, isPaused: false, isPlaying: false,
      currentBiome: 'neon_core', performanceFactor: 1,
      shieldTimer: 0, multiplierTimer: 0, rapidFireTimer: 0,
      slowMoTimer: 0, overdriveTimer: 0, freezeTimer: 0, magnetTimer: 0,
      upgradeData: {},
  ...
  handleResize = () => {
    ...
    if (parent) {
      ...
      const oldW = this.width || cw; const oldH = this.height || ch
      this.width = cw; this.height = ch
      if (this.coreX === 0 || this.coreY === 0) { this.coreX = this.width / 2; this.coreY = this.height / 2 }
      else { this.coreX = (this.coreX / oldW) * this.width; this.coreY = (this.coreY / oldH) * this.height }
    }
  }
  ...
  get threatShakeIntensity(): number { return Math.min(3.5, this.bugs.length * 0.12) }

  private resetEntities() { this.bugs = []; this.hazards = []; this.powerups = []; this.resources = []; this.particleSystem.reset() }
  ...
  private loop = (time: number) => {
    ...
    this.lastTime = time
    if (this.state.isPaused) { this.renderer.draw(); this.animationId = requestAnimationFrame(this.loop); return }
    if (this.hitStopTimer > 0) { this.hitStopTimer -= dt; this.renderer.draw(); this.animationId = requestAnimationFrame(this.loop); return }
    this.update(dt)
    this.renderer.draw()
    this.animationId = requestAnimationFrame(this.loop)
  }

  shake(duration: number, mag: number, dx = 0, dy = 0) { this.shakeTime = duration; this.shakeMagnitude = mag; this.shakeX = dx; this.shakeY = dy }

  private update(dt: number) {
    if (this.state.health <= 0) { this.endGame(); return }
  ...
  private updateTimers(dt: number) {
    ...
    if (this.controlDistortionTimer > 0) { this.controlDistortionTimer -= dt; if (this.controlDistortionTimer <= 0) { this.renderer.isGlitching = false } }
    if (this.shakeTime > 0) this.shakeTime -= dt
    if (this.streakTimer > 0) { this.streakTimer -= dt; if (this.streakTimer <= 0) { this.streakCount = 0 } }
  }

  private updateMetrics(dt: number) {
    this.baseScale += (1 - this.baseScale) * 0.15
    ...
    const safety = Math.min(1, (this.globalTime - this.lastHitTime) / 20)
    const streak = Math.min(1, this.streakCount / 50)
    this.performanceFactor = 0.8 + safety * 0.7 + streak * 1.0
  }

  private updateBugs(dt: number) {
    let ts = this.slowMoTimer > 0 ? 0.3 : 1
    if (this.freezeTimer > 0) ts = 0
    for (let i = this.bugs.length - 1; i >= 0; i--) {
      const bug = this.bugs[i]
      const dx = this.coreX - bug.x; const dy = this.coreY - bug.y; const dsq = dx * dx + dy * dy
      if (dsq < 900) {
        if (this.shieldTimer <= 0) { this.state.health -= GameConfig.player.hitDamage; this.impactFrame = 1; this.lastHitTime = this.globalTime; this.streakCount = 0; this.shake(0.3, 10); this.renderer.chromaticOffset = 15; this.hitStopTimer = 0.1; soundManager.hitBase() }
        else { this.shake(0.2, 5); soundManager.splat() }
        this.particleSystem.spawnExplosion(bug.x, bug.y, bug.color)
        this.bugs.splice(i, 1); continue
      }
      const dist = Math.sqrt(dsq)
      let speed = bug.speed * ts
      let vx = (dx / dist) * speed; let vy = (dy / dist) * speed
      if (bug.type === 'scout' || bug.type === 'swarmer') { const e = Math.sin(this.globalTime * 10 + bug.offsetTime) * (bug.type === 'swarmer' ? 1.2 : 0.5); vx += -vy * e; vy += (dx / dist) * speed * e }
      bug.rotation = Math.atan2(vy, vx) - Math.PI / 2
      bug.x += vx * dt; bug.y += vy * dt
      bug.walkCycle += speed * dt * 0.2
      if (bug.hitTimer > 0) bug.hitTimer -= dt
      if ((this.currentBiome === 'void_abyss' || bug.type === 'phase') && dsq > 10000) {
        bug.lastTeleportTime = (bug.lastTeleportTime || 0) + dt * ts
        if (bug.lastTeleportTime > (bug.type === 'phase' ? 2 : 4)) {
          bug.lastTeleportTime = 0
          this.particleSystem.spawnShockwave(bug.x, bug.y, bug.color, 40)
          const ang = Math.random() * Math.PI * 2; bug.x += Math.cos(ang) * 100; bug.y += Math.sin(ang) * 100
          this.particleSystem.spawnShockwave(bug.x, bug.y, bug.color, 30)
        }
      }
      if (this.currentBiome === 'golden_spire') bug.hp = Math.min(bug.maxHp, Math.max(0, bug.hp + dt * 0.5))
  ...
  private updateHealer(bug: Bug, dt: number, ts: number) {
    ...
    if (bug.healCooldown! > 3) {
      bug.healCooldown = 0; bug.isHealing = true; bug.healEffectTimer = 0.5
      this.particleSystem.spawnShockwave(bug.x, bug.y, '#00ff66', 150)
      this.bugs.forEach(o => { if (o !== bug && o.active) { const odx = o.x - bug.x; const ody = o.y - bug.y; if (odx * odx + ody * ody < 22500) o.hp = Math.min(o.maxHp, o.hp + o.maxHp * 0.2) } })
    }
    if (bug.healEffectTimer && bug.healEffectTimer > 0) { bug.healEffectTimer -= dt * ts; if (bug.healEffectTimer <= 0) bug.isHealing = false }
  }

  private updateBoss(bug: Bug, dt: number, ts: number) {
    if (bug.phase === undefined) { bug.phase = 1; bug.abilityTimer = 0; bug.isShielded = false }
    bug.offsetTime += dt; bug.abilityTimer = (bug.abilityTimer || 0) + dt * ts
    const hpPct = bug.hp / bug.maxHp; const conf = GameConfig.bugs.boss
    if (bug.phase === 1 && hpPct < 0.66) { bug.phase = 2; this.shake(1, 30); soundManager.powerup('overdrive') }
    if (bug.phase === 2 && hpPct < 0.33) { bug.phase = 3; bug.isShielded = true; bug.abilityTimer = 0; this.shake(1.5, 40); this.renderer.chromaticOffset = 20 }
    if (bug.offsetTime > conf.attackRate) {
      bug.offsetTime = 0; this.particleSystem.spawnShockwave(bug.x, bug.y, bug.color, 120)
    ...
    if (bug.phase! >= 2 && bug.abilityTimer! > conf.barrageRate) {
      bug.abilityTimer = 0; soundManager.bossAbility()
      for (let j = 0; j < conf.barrageCount; j++) {
        this.hazards.push({ id: `barrage_${this.globalTime}_${j}`, x: Math.random() * this.width, y: Math.random() * this.height, radius: conf.barrageRadius, type: 'barrage', timer: 0, duration: conf.barrageWarningTime, active: true })
      }
    }
    if (bug.variantId === 'arachne') {
      ...
      if (bug.webTimer > 4) {
        bug.webTimer = 0
        this.hazards.push({ id: `web_${this.globalTime}`, x: Math.random() * this.width, y: Math.random() * this.height, radius: 50, type: 'web', timer: 0, duration: 8, active: true })
      }
    } else if (bug.variantId === 'moth') {
      if (Math.random() < 0.05 * dt * (bug.phase || 1)) {
        this.controlDistortionTimer = 2; this.renderer.isGlitching = true
      }
    } else if (bug.variantId === 'mandible') {
      bug.armor = Math.sin(this.globalTime * Math.PI) > 0 ? 0.8 : 1.0
    }
  }

  private updatePowerups(dt: number) {
    for (let i = this.powerups.length - 1; i >= 0; i--) {
      const p = this.powerups[i]; p.life -= dt
      if (p.life <= 0) { this.powerups.splice(i, 1); continue }
      if (this.magnetTimer > 0) {
        const dx = this.coreX - p.x; const dy = this.coreY - p.y; const d = Math.sqrt(dx * dx + dy * dy)
        p.x += (dx / d) * 400 * dt; p.y += (dy / d) * 400 * dt
      }
    }
  }
  ...
  private updateResources(dt: number) {
    for (let i = this.resources.length - 1; i >= 0; i--) {
      const r = this.resources[i]; if (!r.active) { this.resources.splice(i, 1); continue }
      r.life -= dt
      if (r.life <= 0) { r.active = false; continue }
      const dx = this.coreX - r.x; const dy = this.coreY - r.y; const dsq = dx * dx + dy * dy
      if (dsq < 40000 || this.magnetTimer > 0) {
        const d = Math.sqrt(dsq) || 1
        const factor = this.magnetTimer > 0 ? 1 : (1 - (d / 200))
        r.x -= Math.cos(Math.atan2(dy, dx)) * 500 * factor * dt
        r.y -= Math.sin(Math.atan2(dy, dx)) * 500 * factor * dt
      }
      if (dsq < 900) {
        r.active = false
        const crystalVal = r.type === 'neural_core' ? 50 : r.type === 'flux' ? 10 : r.type === 'alloy' || r.type === 'plasma' ? 5 : 1
  ...
  private updateHazards(dt: number) {
    let isInLava = false; let isInWeb = false
    for (let i = this.hazards.length - 1; i >= 0; i--) {
      const h = this.hazards[i]; h.timer += dt
      if (h.timer >= h.duration) { this.triggerHazard(h); this.hazards.splice(i, 1); continue }
      const dx = h.x - this.coreX; const dy = h.y - this.coreY; const dsq = dx * dx + dy * dy
      const touchR = h.radius + 20
    ...
    if (isInLava && this.shieldTimer <= 0) {
      this.state.health -= dt * 8
      if (Math.random() < 0.1) { this.shake(0.1, 2); this.renderer.impactFlash = Math.max(this.renderer.impactFlash, 0.4) }
    }
  ...
  private triggerHazard(h: Hazard) {
    if (h.type === 'barrage') {
      ...
      const dmgR = h.radius + 30
      if (dsq < dmgR * dmgR && this.shieldTimer <= 0) {
        this.state.health -= GameConfig.player.hitDamage * 1.5
        this.renderer.impactFlash = 1.5; this.shake(0.5, 25); soundManager.hitBase()
      }
    }
  }
  ...
  private spawnResource(x: number, y: number, bugType: string, bonusMultiplier = 1) {
    ...
    for (let i = 0; i < res.count; i++) {
      this.resources.push({
        ...
        type: res.type, color: res.color, active: true, life: 20, size: 6,
      })
    }
  }
  ...
  queueClick(x: number, y: number) { this.inputQueue.push({ x, y }) }
  ...
  private processClick(x: number, y: number) {
    for (let i = this.powerups.length - 1; i >= 0; i--) {
      ...
      if (Math.hypot(p.x - x, p.y - y) < p.size + 20) {
        this.activatePowerup(p.type); this.powerups.splice(i, 1); return
      }
    }
    for (let i = this.bugs.length - 1; i >= 0; i--) {
      const bug = this.bugs[i]
      if (!bug.active || bug.hp <= 0) continue
      const r = bug.isBoss ? bug.size * 1.5 : bug.size + 5
  ...
  damageBug(bug: Bug, amount: number) {
    ...
    if (bug.type === 'boss' && bug.isShielded) {
      this.particleSystem.spawnShockwave(bug.x, bug.y, '#00ffff', 40); soundManager.uiError(); return
    }
    if (bug.type === 'boss') {
      soundManager.bossHit(); const pulse = Math.sin(this.globalTime * 10)
      if (pulse > 0.8) { dmg *= 2; this.particleSystem.spawnShockwave(bug.x, bug.y, '#fff', 60); this.hitStopTimer = 0.05; this.renderer.chromaticOffset = 10 }
      else if (pulse < -0.8) dmg *= 0.5
    }
    bug.hp -= dmg; bug.hitTimer = 0.1
    this.shake(0.05, 2)
    soundManager.shoot()
    ...
    if (bug.hp <= 0) {
      const idx = this.bugs.indexOf(bug); if (idx < 0) return
      this.bugs.splice(idx, 1)
      this.streakCount++; this.streakTimer = 2
      ...
      upgradeSystem.addCrystals(Math.floor(crystalBonus * crystalMulti) + 1)
      saveManager.addCrystalsEarned(Math.floor(crystalBonus * crystalMulti) + 1)

      soundManager.splat()
      this.hitStopTimer = 0.04
      const intens = isBoss ? 4 : (bug.type === 'tank' ? 1.4 : 0.9)
      this.shake(isBoss ? 0.6 : 0.15 * intens, isBoss ? 40 : 8 * intens)
      ...
      if (isBoss) {
        ...
        this.shake(1.5, 60); this.hitStopTimer = 0.2; this.renderer.chromaticOffset = 40; this.impactFrame = 1
      ...
      if (this.overdriveTimer > 0 && this.autoTurretTimer <= 0) {
        this.autoTurretTimer = 0.3
        const target = this.getClosestBug(); if (target) { this.particleSystem.spawnLaser(this.coreX, this.coreY, target.x, target.y, '#ff6600', 3); this.damageBug(target, 1) }
      }
  ...
  private getClosestBug(): Bug | null {
    let closest: Bug | null = null; let minDist = Infinity
    for (const bug of this.bugs) {
      const d = Math.hypot(bug.x - this.coreX, bug.y - this.coreY)
      if (d < minDist) { minDist = d; closest = bug }
    }
    return closest
  }
  ...
  private spawnPowerup(x: number, y: number, force = false) {
    ...
    this.powerups.push({ active: true, x, y, type: pt.type, color: pt.color, icon: pt.icon, life: GameConfig.powerups.life, maxLife: GameConfig.powerups.life, size: 15, collection: pt.collection })
  }
  ...
  activatePowerup(type: string) {
    ...
    switch (type) {
      case 'shield': this.shieldTimer = GameConfig.powerups.duration; break
      case 'multiplier': this.multiplierTimer = GameConfig.powerups.duration; break
      case 'rapid_fire': this.rapidFireTimer = GameConfig.powerups.duration; break
      case 'slow_mo': this.slowMoTimer = GameConfig.powerups.duration; break
      case 'freeze': this.freezeTimer = GameConfig.powerups.duration; break
      case 'magnet': this.magnetTimer = GameConfig.powerups.duration; break
      case 'spike_burst':
        this.shake(0.5, 20); this.particleSystem.spawnShockwave(this.coreX, this.coreY, '#ff3300', 500)
        for (const bug of this.bugs) this.damageBug(bug, 2)
        break
      case 'nuke':
        soundManager.nuke(); this.shake(1.5, 40); this.renderer.chromaticOffset = 30; this.hitStopTimer = 0.15; this.impactFrame = 1
        ...
        break
      case 'overdrive': this.overdriveTimer = GameConfig.powerups.duration; break
    }
    this.emitState()
  }
  ...
  purchaseUpgrade(key: string): boolean {
    ...
    if (result) {
      switch (key) {
        case 'extra_life': this.state.maxHealth += 25; this.state.health = Math.min(this.state.health + 25, this.state.maxHealth); break
        case 'click_power': this.damageMultiplier = upgradeSystem.getClickDamage() / 10; break
        case 'combo_master': this.streakTimer = 2 * upgradeSystem.getComboDecayMultiplier(); break
      }
  ...
  triggerDash(targetX: number, targetY: number) {
    ...
    const dx = targetX - this.coreX; const dy = targetY - this.coreY
    ...
    this.renderer.chromaticOffset = 25; this.impactFrame = 0.35; this.shake(0.4, 12)
  ...
  updateCorePhysics(dt: number) {
    ...
    if (this.dashTimer > 0) {
      ...
      const pushR2 = 120 * 120
      this.bugs.forEach(bug => {
        const dx = bug.x - this.coreX; const dy = bug.y - this.coreY; const dsq = dx * dx + dy * dy
        if (dsq < pushR2) {
          const d = Math.sqrt(dsq) || 1; bug.x += (dx / d) * 450 * dt; bug.y += (dy / d) * 450 * dt
          this.damageBug(bug, 0.4)
        }
      })
      if (this.dashTimer <= 0) this.particleSystem.spawnShockwave(this.coreX, this.coreY, '#00ffff', 160)
    } else {
      const tx = this.width / 2; const ty = this.height / 2
      const dx = tx - this.coreX; const dy = ty - this.coreY; const d = Math.sqrt(dx * dx + dy * dy)
      if (d > 1) {
        const speed = 240; this.coreX += (dx / d) * Math.min(d, speed * dt); this.coreY += (dy / d) * Math.min(d, speed * dt)
      } else { this.coreX = tx; this.coreY = ty }
    }
  }
